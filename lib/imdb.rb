...
class IMDB
  ...
  def perform_search
    ...
    details = doc.css('table.results tr.detailed')
    details.each_with_index do |movie, i|
      info = {}
      ...
      info[:votes]     = movie.css('td.title .rating-list').first[:title].match(/\((\d+\,?\d*)\svotes\)/)[1].gsub(',', '').to_i rescue nil
      ...
      info[:rating]    = nil if info[:rating] =~ /^\-/
      list[number] = info
    end
    return list
  end
